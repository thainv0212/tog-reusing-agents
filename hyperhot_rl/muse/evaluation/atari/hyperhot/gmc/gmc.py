...
from muse.evaluation.atari.hyperhot.gmc.gmc_networks import *
import torch
...
class GMC(LightningModule):
    ...
    def gen_latent(self, image, sound):
        # sound = sound * 2 - 32767
        x = [image, sound]
...
class HyperhotSoundOnly(HyperhotGMC):
    def __init__(self, name, common_dim, latent_dim, loss_type="infonce", finetune=None):
        super(HyperhotSoundOnly, self).__init__(
            name, common_dim, latent_dim, loss_type="infonce"
        )
        self.sound_processor = HyperhotSoundProcessor2(common_dim=common_dim)
        self.image_processor = None
        self.joint_processor = None
        self.processors = [
            self.sound_processor,
...
class HyperhotNullModel(nn.Identity, nn.Module):
