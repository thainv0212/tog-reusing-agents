...
class ObservationNormalizer(nn.Module):
    ...
    def forward(self, obs_dict: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        with torch.no_grad():
            ...
            if self.cfg.encoder_type == "normal":
                if self.should_sub_mean:
                    obs_clone["img"].sub_(self.sub_mean)
                    # obs_clone["sound"].sub_(self.sub_mean)

                if self.should_scale:
                    obs_clone["img"].mul_(1.0 / self.scale)
                    # obs_clone["sound"].mul_(1.0 / self.scale)

                if self.running_mean_std:
                    self.running_mean_std(obs_clone)  # in-place normalization
            elif self.cfg.encoder_type == "gmc":
                min_sound, max_sound = (-32767., 32767.)
                obs_clone["sound"] = (obs_clone["sound"] - min_sound) / (max_sound - min_sound)
