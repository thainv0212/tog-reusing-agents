...
class AtariEnv(gym.Env):
    def __init__(self,
        ...
        if scenario == 'pendulum':
            ...
            self._env = ps2.PendulumSound(original_frequency=scenario_cfg['sound_frequency'],
                                         sound_vel=scenario_cfg['sound_velocity'],
                                         sound_receivers=[ps.SoundReceiver(ps.SoundReceiver.Location[ss]) for ss in
                                                          scenario_cfg['sound_receivers']])
            ...
            self._env = ps3.PendulumSound(original_frequency=scenario_cfg['sound_frequency'],
                                         sound_vel=scenario_cfg['sound_velocity'],
                                         sound_receivers=[ps.SoundReceiver(ps.SoundReceiver.Location[ss]) for ss in
                                                          scenario_cfg['sound_receivers']])
