...
class VizdoomEnv(gym.Env):
    def __init__(
        ...
        self.screen_w, self.screen_h, self.channels = 1920, 1080, 3
        self.screen_resolution = ScreenResolution.RES_1920X1080
    ...
    def _create_doom_game(self, mode):
        ...
        if self.use_sonic_aim_support:
            self.game.add_game_args(
                "-file ./sf_examples/vizdoom/doom/scenarios/sound.wad"
            )
            self.game.set_objects_info_enabled(True)
            # self.game.set_console_enabled(True)
    ...
    def _process_game_step(self, state, done, info):
        ...
        return {"img": observation, "sound": audio_buffer}, done, info
