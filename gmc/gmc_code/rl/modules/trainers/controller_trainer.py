...
class ControllerLearner(LightningModule):
    def __init__(
        ...
        if isinstance(logger, list):
            for l in logger:
                if isinstance(l, SacredLogger):
                    self.sacred_logger = l
                elif isinstance(l, TensorBoardLogger):
                    self.tensorboard_logger = l
...
class DDPGLearner(ControllerLearner):
    ...
    def train_controller(self, post_cb):
        ...
        if self.scenario_config["train_vae"] == 0:
            self.model.eval()
        else:
            self.model.train()
        ...
        while frame_number < self.max_frames:
            ...
            if done:
                ...
                if self.tensorboard_logger is not None:
                    self.tensorboard_logger.experiment.add_scalar("avg_critic_loss",
                                                                  avg_critic_losses.avg,
                                                                  frame_number)
                self.sacred_logger.log_metric(
                ...
                if self.tensorboard_logger is not None:
                    self.tensorboard_logger.experiment.add_scalar(
                        "avg_actor_loss", avg_actor_losses.avg, frame_number
                    )
                ...
                if self.tensorboard_logger is not None:
                    self.tensorboard_logger.experiment.add_scalar(
                        "avg_reward", avg_rewards.avg, frame_number
                    )
                ...
                if self.tensorboard_logger is not None:
                    self.tensorboard_logger.experiment.add_scalar(
                        "avg_episode_total_reward", avg_episode_total_reward.avg, frame_number
                    )
                ...
                if self.tensorboard_logger is not None:
                    self.tensorboard_logger.experiment.add_scalar(
                        "last_episode_total_reward", total_episode_reward, frame_number
                    )
                ...
                if self.tensorboard_logger is not None:
                    self.tensorboard_logger.experiment.add_scalar(
                        "replay_buf_avg_reward", self.memory.stats()[0], frame_number
                    )
    ...
    def val_controller(
        ...
        if self.tensorboard_logger is not None:
            self.tensorboard_logger.experiment.add_scalar(
                "eval_avg_reward", np.mean(total_rewards), frame_number
            )
        ...
        if not end_val:
            self.post_cb.record_eval_checkpoint(controller=self.controller, info=info, model=self.model)
        else:
            self.post_cb.on_train_end(controller=self.controller, info=info, model=self.model)
    ...
    def evaluate_controller(
        ...
        for episode_number in range(max_episodes):
            ...
            # if self.tensorboard_logger is not None:
            #     self.tensorboard_logger.experiment.add_scalar(
            #         "avg_reward", avg_reward, frame_number
            #     )
