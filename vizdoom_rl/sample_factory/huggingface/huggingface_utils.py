...
def generate_replay_video(dir_path: str, frames: list, fps: int, cfg: Config, audio_frames: list = None):
    ...
    if audio_frames is None:
        log.debug(f"No audio frames found, skipping generating sound!")
        return
    log.debug(f"Start saving audio")
    concated_audio = np.concatenate(audio_frames)
    audio_fname = "audio.wav"
    tmp_audio_name = os.path.join(dir_path, audio_fname)
    sf.write(tmp_audio_name, concated_audio, subtype="PCM_16", samplerate=22050)
    log.debug(f"Audio file saved to {tmp_audio_name}")
    log.debug(f"Start merging video and sound together")
    video_name_with_audio = os.path.join(dir_path, "replay_with_audio.mp4")
    os.system(f"ffmpeg -i {video_name} -i {tmp_audio_name} -c:v copy -c:a aac -shortest {video_name_with_audio}")
    log.debug(f"Replay video with audio file saved to {video_name_with_audio}")
