...
def enjoy(cfg: Config) -> Tuple[StatusCode, float]:
    ...
    if cfg.encoder_type == 'gmc':
        actor_critic.encoders[0].gmc.load_state_dict(torch.load(cfg.gmc_model_file)['state_dict'])
    ...
    with torch.no_grad():
        while not max_frames_reached(num_frames):
            ...
            for _ in range(render_action_repeat):
                ...
                frame_audio = obs['sound'][:, -630:, :].detach().cpu().numpy()[0]
                video_audios.append(frame_audio)
    ...
    if cfg.save_video:
        if cfg.fps > 0:
            ...
            fps = 30
        generate_replay_video(experiment_dir(cfg=cfg), video_frames, fps, cfg, audio_frames=video_audios)
