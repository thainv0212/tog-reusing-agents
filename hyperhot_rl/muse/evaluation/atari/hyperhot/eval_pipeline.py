...
from muse.scenarios.multimodal_atari import hyperhot_env, hyperhot_env2, hyperhot_env3, hyperhot_env4
...
def get_gmc_model(config, cuda):
    gmc_config = config['eval_pipeline']
    gmc = vae_utils.load_gmc_checkpoint(config['gmc']['train_config'], gmc_config['vae_file'], cuda)
    return gmc, (-32767, 32767)


def get_dqn_model(config, cuda):
    eval_pipeline_config = config['eval_pipeline']
    learner = config['gmc']['train_config']['learner']
    dqn, _ = dqn_utils.load_checkpoint(eval_pipeline_config['dqn_file'], cuda, learner)
...
def eval(vae, sound_normalization, dqn, _config, _run):
    ...
    h = None
    env = None
    if _config['gmc']['env'] == 'hyperhot':
        env = hyperhot_env.HyperhotEnv(
    ...
    elif _config['gmc']['env'] == 'hyperhot2':
        env = hyperhot_env2.HyperhotEnv(
            num_enemies=hyperhot_config['n_enemies'],
            pacifist_mode=hyperhot_config['pacifist_mode'],
            time_limit=hyperhot_config['time_limit'],
            sound_receivers=[
    ...
    elif _config['gmc']['env'] == 'hyperhot3':
        env = hyperhot_env3.HyperhotEnv(
            num_enemies=hyperhot_config['n_enemies'],
            pacifist_mode=hyperhot_config['pacifist_mode'],
            time_limit=hyperhot_config['time_limit'],
            sound_receivers=[
    ...
    elif _config['gmc']['env'] == 'hyperhot4':
        env = hyperhot_env4.HyperhotEnv(
            num_enemies=hyperhot_config['n_enemies'],
            pacifist_mode=hyperhot_config['pacifist_mode'],
            time_limit=hyperhot_config['time_limit'],
            sound_receivers=[
        ...
        raise ValueError('Unknown hyperhot environment')
    # print("Loading environment from", h)
    # env = h.HyperhotEnv(
    #     num_enemies=hyperhot_config['n_enemies'],
    #     pacifist_mode=hyperhot_config['pacifist_mode'],
    #     time_limit=hyperhot_config['time_limit'],
    ...
    #         for ss in hyperhot_config['sound_receivers']
    #     ])
    print(hyperhot_config['sound_receivers'])
...
def main(_config):
    ...
    vae, sound_normalization = get_gmc_model(_config, cuda)
